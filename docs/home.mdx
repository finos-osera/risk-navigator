...
- An OSS library-risk browser at `oss-risk/browser.html`, seeded with FINOS
  OSERA backpatch library scorecards.
...
The OSS library-risk browser is hosted alongside the tool:

```text
https://finos-osera.github.io/risk-navigator/oss-risk/browser.html
```

On local and custom-domain deployments the same browser is served from
`/oss-risk/browser.html`. See [OSS Library Risk](./oss-risk) for the source
model, collector behavior, and provenance notes.
